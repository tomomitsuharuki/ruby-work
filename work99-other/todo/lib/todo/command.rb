...
module Todo
	...
	class Command
		...
		def display_format(id, name, content, status)
			name_lenght = 20 - full_width_count(name)
			content_lenght = 40 - full_width_count(content)
			[id.to_s.rjust(4), name.ljust(name_lenght), content.ljust(content_lenght), status.center(10)].join(' | ')
		end
